...
def semantic_validation_engine(data, mode, api_key):
    ...
    if mode == "دخول مباشر (محاكي ذكي)" or not api_key:
        ...
        # أ. المهن الطبية والقانونية تتطلب بكالوريوس على الأقل
        if any(kw in job for kw in medical_keywords + legal_keywords):
            if edu not in ["بكالوريوس", "ماجستير", "دكتوراه"]:
                return {"score": 0.15, "reasoning": f"فشل المطابقة المهنية: مهنة '{job}' تتطلب حتماً مؤهلاً جامعياً تخصصياً (بكالوريوس فأعلى)."}
            if age < 23:
                return {"score": 0.3, "reasoning": f"تنبيه سن قانوني: مهنة '{job}' تتطلب ترخيصاً مهنياً لا يمنح عادة قبل سن 23-25 عاماً."}
...
if app_page == "🔍 فحص السجلات الميدانية":
    st.markdown('<h1 class="main-title">🛡️ التدقيق الدلالي الشامل</h1>', unsafe_allow_html=True)
    ...
    with st.container():
        ...
        with col1:
            ...
            job = st.text_input("المسمى الوظيفي (أدخل أي مهنة)", "طبيب استشاري")
        with col2:
    ...
    with st.expander("🛠️ كيف يعمل الفحص الشامل؟"):
        ...
        2. **المطابقة المهنية:** يحلل الكلمات المفتاحية للمهنة ويربطها بالمتطلبات الأكاديمية (طب، قانون، هندسة..).
